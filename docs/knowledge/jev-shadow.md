...
`ConsolidationResult.shadow` 提供状态 `ok`、`not_allowed` 或 `degraded`、尝试/成功/失败/预算跳过数及逐对差异。每对只有运行内随机盐生成的 fingerprint、两个关系类别、Jev 概率/供应商 confidence、模型与 rubric 版本、耗时或安全失败代码；不含事实原文、记忆 ID 或 API 密钥。每次请求接收本轮剩余时间作为更严格的 deadline（不超过客户端自身上限）；预算耗尽、超时与服务错误只降低旁路状态，现有主判仍可照常规划和执行。报告是一次运行内的审计结果，调用方应根据自己的保留策略保存；不要把报告中的 Jev 选择反向写入治理决策。
